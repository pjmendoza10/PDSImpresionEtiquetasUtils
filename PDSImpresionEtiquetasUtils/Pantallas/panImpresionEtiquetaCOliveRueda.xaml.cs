...
namespace PDSImpresionEtiquetasUtils.Pantallas
{
    ...
    /// </summary>
    public partial class panImpresionEtiquetaCOliveRueda : UserControl, IPantallasContenedor
    {
        private panImpresionEtiquetaCOliveRueda_ViewModel _viewmodel;

        public panImpresionEtiquetaCOliveRueda()
        {
            InitializeComponent();

            PantallaAnterior = null;
            _viewmodel = (Pantallas.panImpresionEtiquetaCOliveRueda_ViewModel)this.DataContext;
        ...
        private void txtCodArticulo_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.Key == Key.Enter & (sender as TextBox).AcceptsReturn == false)
            {
                ...
                if (e.Key == Key.Enter) this._viewmodel.BuscarMaterialCliente_Command_Execute();
        ...
        private void txtLoteArticulo_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.Key == Key.Enter & (sender as TextBox).AcceptsReturn == false)
            {
                ...
                {
                    this._viewmodel.BuscarPorCliente_Command_Execute();
                }
                else
                {
